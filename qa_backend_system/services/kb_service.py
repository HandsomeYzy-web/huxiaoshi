...
from api.dependencies import invalidate_kb_access_cache
...
class KBService:
    ...
    def list_kbs(self, db: Session, accessible_kb_ids: list[int]) -> list[KnowledgeBase]:
        """列出用户可访问的知识库， accessible_kb_ids 由调用方通过角色层查询传入。"""
        if not accessible_kb_ids:
            return []
        return KBRepo(db).get_kbs_by_ids(accessible_kb_ids)
